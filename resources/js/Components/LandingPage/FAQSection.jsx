...
const FAQSection = () => {
    ...
    const handleCategoryChange = (category) => {
        ...
        { name: 'Pricing', icon: '💰', count: faqs.filter(faq => faq.category === 'Pricing').length },
        { name: 'Features', icon: '⚡', count: faqs.filter(faq => faq.category === 'Features').length },
        { name: 'Security', icon: '🔒', count: faqs.filter(faq => faq.category === 'Security').length },
        { name: 'Technical', icon: '🛠️', count: faqs.filter(faq => faq.category === 'Technical').length },
        { name: 'Reports', icon: '📊', count: faqs.filter(faq => faq.category === 'Reports').length },
        { name: 'Support', icon: '🎧', count: faqs.filter(faq => faq.category === 'Support').length },
        { name: 'Setup', icon: '⚙️', count: faqs.filter(faq => faq.category === 'Setup').length },
        { name: 'Enterprise', icon: '🏢', count: faqs.filter(faq => faq.category === 'Enterprise').length },
        { name: 'General', icon: '📱', count: faqs.filter(faq => faq.category === 'General').length }
    ];
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="text-center mb-16">                    <div className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-100 to-indigo-100 rounded-full mb-6 animate-fade-in-up">
                        <span className="text-blue-700 font-semibold text-sm">
                            💡 Jawaban untuk 20 pertanyaan paling sering ditanyakan
                        </span>
                    </div>
                        ...
                        <div className="mb-6 animate-fade-in-up-delay-3">
                            <div className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-amber-100 to-yellow-100 rounded-full">
                                <span className="text-amber-600 font-medium text-sm">
                                    ⭐ Menampilkan {filteredFaqs.filter(faq => faq.popular).length} pertanyaan paling populer dari business owner
                                </span>
                            </div>
                        </div>
                    )}
                                            ...
                                            <div className="border-t border-gray-100 pt-4">
                                                ...
                                                <div className="mt-4 flex items-center justify-between">
                                                    ...
                                                    <div className="text-xs text-gray-400">
                                                        Helpful? 
                                                        <button className="ml-2 text-green-500 hover:text-green-600">👍</button>
